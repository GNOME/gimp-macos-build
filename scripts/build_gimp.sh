...
function usage() {
    ...
    echo "  --gimp-branch branch"
    echo "      specify the branch of gimp to build (default: master). Can't be"
    echo "      used with --gimp-tag"
    echo "  --gimp-tag tag"
    echo "      specify the tag of gimp to build. Can't be used with --gimp-branch."
    echo "      This should be used for tagged releases."
...
if [ ! -z "${BRANCH}" ] && [ ! -z "${TAG}" ]; then
    echo "ERROR: Can't specify both --gimp-branch and --gimp-tag"
    exit 1
fi
