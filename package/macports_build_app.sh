...
GIMP_VERSION="$(${PREFIX}/bin/gimp-3.1 --version 2>/dev/null | grep 'GNU Image Manipulation Program version' | sed 's|GNU Image Manipulation Program version ||')"
# for gtk-mac-bundler
...
pushd ${PACKAGE_DIR}/GIMP.app/Contents/MacOS || exit 1
  ln -s gimp-console-3.1 gimp-console
  ln -s gimp-debug-tool-3.1 gimp-debug-tool
