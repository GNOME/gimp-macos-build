...
# must be global to support functions being called without it
global darwin_version
...
ui_error "darwin_version: ${darwin_version}"
...
proc legacysupport::get_cpp_flags {} {
    global os.platform darwin_version prefix
    if {${os.platform} eq "darwin" && ${darwin_version} <= [option legacysupport.newest_darwin_requires_legacy]} {
...
proc legacysupport::get_library_link_flags {} {
    global prefix os.platform darwin_version
    if {${os.platform} eq "darwin" && ${darwin_version} <= [option legacysupport.newest_darwin_requires_legacy]} {
...
proc legacysupport::set_label_environment_vars { } {
    global os.platform darwin_version
...
proc legacysupport::relink_libSystem { exe } {
    global darwin_version prefix
    if { ${darwin_version} <= [option legacysupport.newest_darwin_requires_legacy] } {
